...
def test_최근에_들어왔으면_조용하다() -> None:
    assert StaleDataTrigger().check(_seen(10), NOW) is None


def test_하루를_통째로_건너뛰면_감지한다() -> None:
    insight = StaleDataTrigger().check(_seen(40), NOW)
    assert insight is not None
    assert insight.severity is Severity.NOTABLE


def test_사흘_넘게_끊기면_심각하다() -> None:
    insight = StaleDataTrigger().check(_seen(80), NOW)
    assert insight is not None
...
def test_한_번도_안_들어온_건_멈춤이_아니다() -> None:
    ...
    assert StaleDataTrigger().check([], NOW) is None


def test_종류마다_이름이_갈린다() -> None:
    """이름이 겹치면 쿨다운을 공유해서 한쪽이 다른 쪽을 막아버린다."""
    sleep = StaleDataTrigger(kind="sleep_hours")
    steps = StaleDataTrigger(kind="step_count")
