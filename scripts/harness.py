...
DEFAULTS = {
    ...
    "ruff": True,
    # 정적 검사 대상. run_tests 는 pytest 그대로 쓰되, 완료 판정에 ruff·mypy 를 묶는다.
    "static_paths": ["src", "app", "tests"],
    "typecheck_paths": ["src", "app"],
}
...
def run_static() -> bool:
    """ruff + mypy. CI가 보는 것과 **같은 것**을 본다.

    이걸 harness 밖에 두면 '완료'가 두 개가 된다 — harness all 은 통과했는데
    CI는 빨간 상황. 같은 일을 하는 판정이 두 곳에 있으면 반드시 갈라진다.
    """
    checks = [
        (["-m", "ruff", "check", *CFG["static_paths"]], "ruff check"),
        (["-m", "ruff", "format", "--check", *CFG["static_paths"]], "ruff format"),
        (["-m", "mypy", *CFG["typecheck_paths"]], "mypy"),
    ]
    print("\n" + "\u2500" * 46 + "\n  정적 검사")
    ok = True
...
def run_one(target: str, gc: bool, diff: bool) -> bool:
    ...
    if result["status"] == "no_tests":
        ...
        if result["status"] != "pass":
            print("\n".join("     " + ln for ln in result["output"].splitlines()[-25:]))
